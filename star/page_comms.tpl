			...
			<div class="card-body">
				...
				<table align="center" border="0" style="border-collapse:collapse;" id="group.details" cellpadding="3" class="table">
					<tr>
						<td valign="top">
							<div class="rowdesc">
								<i class="text-primary mdi mdi-help-circle" data-toggle="tooltip" data-placement="bottom" title="输入您想说的文字。"></i> 评论
							</div>
						</td>
					</tr>
					<tr>
...
<div class="row">
	<div class="col-lg-12 grid-margin">
		<div class="card">
			<div class="card-body">
				<h4 class="lead">
				交流阻挡列表概述 <a class="btn btn-outline-primary btn-rounded btn-fw" style="width:20px;height:20px;padding:0px;line-height:18px;" href="index.php?p=commslist&hideinactive={if $hidetext == 'Hide'}true{else}false{/if}{$searchlink|htmlspecialchars}" title="{$hidetext} Inactive">{$hidetext} 不活跃</a></h3>
				<p>总计封禁: {$total_bans}</p>
				{php} require (TEMPLATES_PATH . "/admin.comms.search.php");{/php}
				{php} $pageName="comms"; include("./themes/star/progressBansComms.php");{/php}
				<div align="right" style="margin-top: 3px; font-size:7pt">SourceComms 插件 &#038;	与 SourceBans 的集成，作者 <a href="https://github.com/ppalex7" target="_blank">Alex</a></div>
				<br />
					...
					<table class="table table-hover tbl-sm">
						<thead>
							<tr>
								{if $view_bans}
								<th>
									<div class="ok" style="display:none;height:16px;width:16px;cursor:pointer;" title="选择全部" name="tickswitch" id="tickswitch" onclick="TickSelectAll()"></div>
								...
								{/if}
								<th width="12%" class="text-center">MOD/类型</th>
								<th width="14%" class="text-center">日期</th>
								<th>Player</th>
								{if $view_comments}
								<th class="text-right">历史阻挡</th>
								{/if}
								{if !$hideadminname}
								<th width="15%">管理员</th>
								{/if}
